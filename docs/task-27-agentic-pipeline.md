...
6. **Before writing anything, every agent answers three questions, in this order.** Added after Milestone 2, at the user's request, as an explicit statement of what principles 2–5 already implied separately but never said as one discipline:
   - **Is this fact true?** — validation (`pipeline/validation.ts`) and, where the fact's tier demands it, independent corroboration (`docs/truth-verification-layer.md`). An agent that cannot answer this affirmatively does not proceed to the next question.
   - **How sure am I?** — the confidence engine (`pipeline/confidence.ts` §7): source-tier adjustment, corroboration boost, recency decay, floored and ceilinged. This is a number, not a feeling, and it is computed the same way for every agent (principle 4).
   - **Can I prove it?** — the evidence layer (§5, below): a `sourceDomain`/`sourceUrl` that survives sanitisation, and a `rawExcerpt` wherever the source material offers one. A fact with no answer to this question is a fact with no answer to the first one either — "true" and "provable" are not actually separable in a pipeline that has no independent way to know the truth except through what it can cite.

   These three questions are not a new mechanism — every piece of machinery needed to answer them already exists in `validation.ts`, `confidence.ts`, and the Evidence Layer. What's new is that they are now named as the question every agent's implementation should be read against, including in code review of future milestones: a line of agent code that writes a fact without having gone through all three, in order, is a bug regardless of what else it gets right.
...
### 5.1 A gap found while writing the Milestone 3 proposal, and what it means going forward

The "not a new database table by necessity" escape hatch above holds for facts that map onto a row in a product table (`contacts`, `timeline_events`, `intelligence_items`) — those rows already carry their own `source_domain`/`source_url`/`confidence` columns, so the Evidence Record's fields *are* the row. It does **not** hold for a fact that maps onto a bare scalar column on `athletes` (e.g. `personal_best`, `world_rank`) — there is no row for the record to be. `ResultsAgent` (Milestone 3) is the first agent for which this actually matters, because all five of its owned fields are exactly that kind of scalar column, and nothing has written to the append-only log described in the paragraph above yet (as of Milestone 2, `evidence_log` has zero writers in the codebase). This was not a problem any prior milestone had to solve, so it was not caught until now.

**Resolution for Milestone 3, kept deliberately minimal:** `ResultsAgent` persists its run's Evidence Records as a JSON array in `evidence_log.raw_extraction` (an existing `text` column, unchanged schema) — one row per agent invocation, same shape `legacyMonolithAgent` would use if it wrote here at all. This satisfies principle 2 (every fact traceable to a source and a confidence computation) without a migration. It does **not** make individual facts independently queryable by claim — reading "what backed Peter Bol's `personal_best` on this run" means reading and parsing that one JSON blob, not querying a table with a `WHERE claim = 'personalBest'`.

**What this means for customer-facing evidence and verification-status surfacing (added at the user's request, design-only, not built in Milestone 3):**

- **Evidence visibility.** The goal is a UI element attached to any stored fact showing what backs it — e.g. a mark, its confidence, and a checkmark per corroborating source domain. Everything the badge needs already exists *per-record* (`sourceDomain`, `sourceUrl`, `confidenceBase`, the tier/corroboration/recency breakdown computed in `confidence.ts`) — what's missing is a place to store more than one agent-invocation's Evidence Records per athlete in a form queryable by claim, so the UI can ask "show me everything that backs `personal_best` right now" without parsing blobs across every past run. That place is a future, dedicated per-claim evidence table (tentatively `fact_evidence`: one row per stored Evidence Record, foreign-keyed to athlete, with `claim`, `sourceDomain`, `sourceUrl`, `tier`, `confidenceBase`, `adjustedConfidence`, `rawExcerpt`, `retrievedAt`) — additive, no change to any existing table, but a genuine new table, not "free" the way the escape-hatch case is.
- **"Needs verification."** `docs/truth-verification-layer.md` §5 already designed almost exactly this — a `verificationStatus` per Tier 2/3 fact (`single_source`, `corroborated`, `contested`, `unverified`) and an explicit count of unconfirmed candidates. What that document scoped as admin/audit-only, the user's Addition 2 extends to a customer-facing count (e.g. an `IntelligenceHealthPanel` line like "3 facts awaiting confirmation"). These are not the same count: a `single_source` Tier 2 fact is already shown to users today (per that document, "at reduced confidence... labelled as single-source"), so counting *those* on the customer-facing panel is a natural, low-risk extension of an already-approved design. An `unverified` Tier 3 *unpublished candidate* is, by that same document, "never shown to any user" — surfacing a count of those on a customer panel would be a scope change to an approved document, not a free extension, and is called out here rather than assumed.
- **Both of the above are additive** to everything built through Milestone 2 and everything proposed for Milestone 3 — no existing column, table, or write path needs to change shape to accommodate them later. They are not implemented now. A future milestone (tentatively named "Evidence Ledger & Verification Surfacing," positioned after enough retrieval agents exist to know the real shape of per-claim evidence across more than one fact domain, and coupled to whatever subset of `docs/truth-verification-layer.md` is approved for real implementation at that time) is the right place for them.

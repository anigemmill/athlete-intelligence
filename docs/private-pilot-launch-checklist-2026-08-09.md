...
| `FOUNDER_EMAIL` | Grants admin access to `/api/admin/*` and the `/admin` nav link | **Now actually required** — see below. Without it, nobody gets admin access, by design. |
...
**Update — the `FOUNDER_EMAIL` finding from this checklist's first draft is fixed.** It previously wasn't read from the environment; the admin email was hardcoded in two backend files and duplicated a third time in the frontend. All three now read from a single shared `isFounderEmail()` check (`artifacts/api-server/src/lib/founderAccess.ts`) that reads `process.env.FOUNDER_EMAIL` on every call. **You must set the `FOUNDER_EMAIL` secret in Replit before deploying** — it fails safe: if it's unset or blank, admin access is denied to everyone, including you, rather than silently granted to no one or the old hardcoded address. Set it to the exact email your Clerk account signs in with (case doesn't matter, matching is case-insensitive).

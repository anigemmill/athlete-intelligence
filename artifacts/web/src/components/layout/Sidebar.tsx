...
const mockAgents = [
  { id: 1, name: "Lola Anderson", sport: "100m Sprint", status: "active", alert: true },
  { id: 2, name: "Marcus Webb", sport: "Decathlon", status: "active", alert: false },
  { id: 3, name: "Priya Nair", sport: "5000m", status: "active", alert: true },
  { id: 4, name: "James Kowalski", sport: "High Jump", status: "active", alert: false },
  { id: 5, name: "Sophie Chen", sport: "400m Hurdles", status: "paused", alert: false },
];
...
export function Sidebar({ activePage = "dashboard" }: SidebarProps) {
  ...
  // Use API if available, fallback to mock
  const { data: athletesData } = useListAthletes();
  const agents = athletesData || mockAgents;
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ overflow: "auto", flex: 1, padding: "0 10px" }} className="hide-scrollbar">
          {agents.map((agent: any) => (
            <Link key={agent.id} href={`/athletes/${agent.id}`}>
              <div
                style={{
                  display: "flex",
                  alignItems: "center",
      ...
      <div style={{
        ...
        <button 
          onClick={() => signOut()}
          className="hover:bg-[rgba(255,255,255,0.1)] p-1 rounded transition-colors"
          title="Sign out"
        >
          <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="rgba(252,250,250,0.28)" strokeWidth={2} className="hover:stroke-[rgba(252,250,250,0.8)] transition-colors">

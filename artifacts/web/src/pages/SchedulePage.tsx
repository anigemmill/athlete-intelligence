...
// A small palette of brand-friendly colours to distinguish athletes in the list
const ATHLETE_COLORS = [
  "#344F9F", "#E75D50", "#2B9E6E", "#C07D25", "#6B4FA3",
  "#2176AE", "#D4660A", "#1A7D5C", "#8B2FC9", "#0E7490",
];
...
// ── API type (matches what /api/competitions actually returns) ─────────────────
...
function normalise(c: ApiComp): Comp {
  return {
    ...c,
    color: athleteColor(c.athleteName),
    initials: athleteInitials(c.athleteName),
  };
}
...
function CountdownDigit({ value, label }: { value: number; label: string }) {
  return (
    <div className="flex flex-col items-center">
      <div className="text-4xl font-bold tabular-nums leading-none tracking-tight" style={{ color: "#FEEEEE", fontVariantNumeric: "tabular-nums" }}>
...
function Separator() {
  return <div className="text-3xl font-light mb-4 select-none" style={{ color: "rgba(252,250,250,0.30)" }}>:</div>;
}
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: comp.color + "28", color: comp.color, border: `1.5px solid ${comp.color}30` }}
      >
        {comp.initials}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          <span className="text-[13px] font-semibold text-[#1C1F3A] truncate">{comp.meetName}</span>
          {isNext && (
      ...
      <AppLayout activePage="schedule">
        <div className="flex flex-col h-full bg-[#FCFAFA]">
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="w-6 h-6 border-2 border-[#344F9F] border-t-transparent rounded-full animate-spin" />
          </div>
        </div>
      </AppLayout>
    );
        ...
        <div className="flex flex-col h-full bg-[#FCFAFA]">
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <h3 className="text-[15px] font-semibold text-[#1C1F3A] mb-1">No upcoming competitions</h3>
              <p className="text-[13px] text-[#6B7080]">Competitions linked to your monitored athletes will appear here automatically after populating athlete data.</p>
            </div>
          </div>
        </div>
      </AppLayout>
    ...
    <AppLayout activePage="schedule">
      <div className="flex flex-col h-full bg-[#FCFAFA] overflow-hidden">
        ...
        <div
          ...
          style={{ background: "linear-gradient(135deg, #293055 0%, #1e2440 100%)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}
        >
          <div className="flex flex-col gap-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              <Timer size={13} style={{ color: "#E75D50" }} />
              <span className="text-[11px] font-bold uppercase tracking-widest" style={{ color: "#E75D50" }}>Next Race</span>
            </div>
          ...
          {!countdown.past ? (
            ...
            <span className="text-[13px] shrink-0" style={{ color: "rgba(252,250,250,0.45)" }}>Completed</span>
          )}
        </div>
        ...
        <div className="flex-1 flex overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto hide-scrollbar px-4 py-4">
              {Object.entries(grouped).map(([month, comps]) => (
                <div key={month} className="mb-5">
                  <div className="text-[11px] font-bold text-[#A0A8C0] uppercase tracking-widest mb-2 px-1">{month}</div>
          ...
          {selected && (
            <div className="flex-1 overflow-y-auto hide-scrollbar px-8 py-6 bg-[#FCFAFA]">
              <div className="flex items-start justify-between mb-6">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[11px] text-[#A0A8C0] font-medium">{selected.status === "upcoming" ? "Upcoming" : "Completed"}</span>
                  </div>
                  <h2 className="text-[22px] font-semibold text-[#1C1F3A] leading-tight mb-1">{selected.meetName}</h2>
              ...
              {/* Athlete pill */}
              <div className="flex items-center gap-3 mb-5 p-4 rounded-xl border border-[#DCE2EF] bg-white shadow-sm">
                <div
                ...
                <div>
                  <div className="text-[13px] font-semibold text-[#1C1F3A]">{selected.athleteName}</div>
                  <div className="text-[12px] text-[#8A90A8]">{selected.event}</div>
                </div>
                <div className="ml-auto text-right">
                  <div className="text-[12px] font-medium text-[#1C1F3A]">
                    {new Date(selected.date).toLocaleDateString("en-GB", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}
                  </div>
                  {selected.result && (
                    <div className="text-[12px] text-emerald-600 font-semibold mt-0.5">Result: {selected.result}</div>
                  )}
                </div>
              </div>
              ...
              >
                ...
                <div className="flex items-end gap-2.5">
                  {selectedCountdown.past || selected.status !== "upcoming" ? (
                    <span className="text-[13px] font-medium" style={{ color: "rgba(252,250,250,0.55)" }}>

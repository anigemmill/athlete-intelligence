...
function DashboardPreview() {
  const athletes = [
    ...
    { initials: "MP", name: "Maya Petrov", sport: "High Jump", rank: 7, rankDelta: +3, confidence: 88, status: "Rising", statusColor: "#344F9F", bg: "rgba(52,79,159,0.15)", border: "rgba(52,79,159,0.3)" },
  ...
  const feed = [
    ...
    { initials: "MP", color: "#344F9F", bg: "rgba(52,79,159,0.2)", name: "Maya Petrov", tag: "Intel Update", tagColor: "#8FA4E8", tagBg: "rgba(52,79,159,0.15)", text: "Cleared 1.96m in training — first time above personal best in 18 months.", time: "1d ago" },
  ];
    ...
    <div className="absolute inset-0 flex items-center justify-center p-6 pointer-events-none select-none">
      ...
      <div className="w-full max-w-[580px] rounded-2xl overflow-hidden shadow-[0_32px_80px_rgba(0,0,0,0.55)] border border-white/[0.08]" style={{ background: "#FCFAFA" }}>
        ...
        <div className="flex" style={{ height: 380 }}>
          ...
          {/* Sidebar */}
          <div className="w-[120px] flex-shrink-0 flex flex-col py-3 border-r" style={{ background: "#293055", borderColor: "rgba(255,255,255,0.06)" }}>
            <div className="px-3 mb-4 flex items-center gap-1.5">
              <div className="w-5 h-5 rounded flex items-center justify-center" style={{ background: "#E75D50" }}>
            ...
            <div className="px-2 space-y-0.5 flex-1">
              {navItems.map((item) => (
                <div key={item.label} className="flex items-center justify-between px-2 py-1.5 rounded-md" style={{ background: item.active ? "rgba(231,93,80,0.15)" : "transparent" }}>
                  <div className="flex items-center gap-1.5">
                    <span style={{ color: item.active ? "#E75D50" : "rgba(255,255,255,0.4)" }}>{item.icon}</span>
                    <span className="text-[9px] font-medium" style={{ color: item.active ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.4)" }}>{item.label}</span>
                  </div>
                  {item.badge && (
                    <div className="w-3.5 h-3.5 rounded-full flex items-center justify-center text-[6px] font-bold text-white" style={{ background: "#E75D50" }}>{item.badge}</div>
                  )}
                </div>
              ))}
            </div>
            <div className="px-3 pt-2 border-t mt-2" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
              <div className="flex items-center gap-1.5">
                <div className="w-5 h-5 rounded-full flex items-center justify-center text-[7px] font-bold" style={{ background: "rgba(231,93,80,0.3)", color: "#E75D50" }}>AN</div>
                <div>
          ...
          <div className="flex-1 overflow-hidden flex flex-col">
            ...
            <div className="px-4 pt-3 pb-2 border-b flex items-center justify-between" style={{ borderColor: "#DCE2EF" }}>
              <div>
                ...
                <div className="text-[8px] text-[#8A90A8]">Saturday, 19 July 2026</div>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="px-2 py-1 rounded-md text-[8px] font-medium text-white" style={{ background: "#E75D50" }}>+ Add Athlete</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-4 gap-2 px-4 py-2 border-b" style={{ borderColor: "#DCE2EF" }}>
              {[
                ...
                { label: "Alerts", value: "12", sub: "unread", color: "#E75D50" },
            ...
            <div className="flex-1 grid grid-cols-5 gap-0 overflow-hidden">
              ...
              <div className="col-span-3 border-r overflow-hidden" style={{ borderColor: "#DCE2EF" }}>
                <div className="px-3 py-2 border-b flex items-center justify-between" style={{ borderColor: "#DCE2EF" }}>
                  ...
                  <span className="text-[7px] text-[#E75D50] font-medium">View all →</span>
                </div>
                  ...
                  {athletes.map((a) => (
                    <div key={a.name} className="flex items-center gap-2 px-3 py-1.5">
                      ...
                      <div className="text-right shrink-0">
                        <div className="text-[7px] font-medium text-[#8A90A8]">#{a.rank} <span style={{ color: a.rankDelta > 0 ? "#10B981" : a.rankDelta < 0 ? "#E75D50" : "#8A90A8" }}>{a.rankDelta > 0 ? `↑${a.rankDelta}` : a.rankDelta < 0 ? `↓${Math.abs(a.rankDelta)}` : "—"}</span></div>
                ...
                <div className="px-3 py-2 border-t" style={{ borderColor: "#DCE2EF" }}>
                  ...
                  <div className="flex items-end gap-0.5 h-8">
                    {barHeights.map((h, i) => (
                      <div key={i} className="flex-1 rounded-t-[2px]" style={{ height: `${h}%`, background: i === barHeights.length - 1 ? "#E75D50" : i >= barHeights.length - 4 ? "rgba(52,79,159,0.5)" : "rgba(52,79,159,0.2)" }} />
                    ))}
                  </div>
                </div>
              </div>
...
// ── Sign-in form skeleton shown while Clerk loads ─────────────────────────────
...
function ClerkSkeleton() {
  return (
    <div className="w-full animate-pulse space-y-3">
      ...
      <div className="h-11 rounded-lg bg-[#E75D50]/20" />
    </div>
  );
}
...
export default function SignInPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FCFAFA] text-[#1C1F3A] flex font-sans selection:bg-[#E75D50]/20 athlete-intelligence-root">
      ...
      <div className="w-full lg:w-[480px] flex-shrink-0 flex flex-col justify-between p-8 sm:p-14 lg:p-16 relative z-10">
            ...
            <SignIn
              ...
              appearance={{
                elements: {
                  ...
                  formFieldInput: "border-[#DCE2EF] focus:ring-[#E75D50] focus:border-[#E75D50] rounded-lg",
                  ...
                  footerActionLink: "text-[#E75D50] hover:text-[#D04840] font-medium",
        ...
        <div>
          ...
          <div className="mt-3 text-center lg:text-left">
            <a href="/contact" className="text-sm font-medium text-[#E75D50] hover:text-[#D04840] transition-colors inline-flex items-center gap-1">
      ...
      <div className="hidden lg:block flex-1 relative overflow-hidden" style={{ background: "#1a1f38" }}>
        {/* Ambient glows */}
        <div className="absolute top-0 right-0 w-[500px] h-[400px] rounded-full opacity-20 blur-[120px]" style={{ background: "#E75D50" }} />
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] rounded-full opacity-15 blur-[120px]" style={{ background: "#344F9F" }} />

        {/* Label */}
        <div className="absolute top-8 left-8 z-20">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[11px] font-semibold" style={{ background: "rgba(231,93,80,0.15)", border: "1px solid rgba(231,93,80,0.3)", color: "#E75D50" }}>
            <div className="w-1.5 h-1.5 rounded-full bg-[#E75D50] animate-pulse" />

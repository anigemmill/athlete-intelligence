          ...
          <div className="flex items-center gap-3">
            <div className="relative group">
              ...
              <input 
                ...
                className="bg-[#16161a] border border-[#DCE2EF] rounded-lg pl-9 pr-4 py-2 text-sm text-[#3D426A] placeholder:text-[#9097B0] focus:outline-none focus:border-[#E75D50]/50 focus:ring-1 focus:ring-[#E75D50]/50 transition-all w-64"
              />
        ...
        <div className="flex-1 overflow-y-auto p-8 ai-scrollbar">
          ...
          <div className="grid grid-cols-4 gap-4 mb-8">
            <div className="ai-card p-5">
              ...
              <div className="text-2xl font-semibold text-[#1C1F3A]">42</div>
            ...
            <div className="ai-card p-5">
              ...
              <div className="text-2xl font-semibold text-[#1C1F3A]">18</div>
              <div className="text-xs text-[#8A90A8] mt-2 flex items-center gap-1">
            ...
            <div className="ai-card p-5 relative overflow-hidden group cursor-pointer">
              ...
              <div className="flex items-start justify-between mb-3 relative z-10">
                <div className="text-sm font-medium text-[#6B7080] group-hover:text-[#3D426A] transition-colors">Unread Alerts</div>
                <Bell className="w-4 h-4 text-[#E75D50]" />
              </div>
              <div className="text-2xl font-semibold text-[#1C1F3A] relative z-10">3</div>
            ...
            <div className="ai-card p-5">
              ...
              <div className="text-2xl font-semibold text-[#1C1F3A]">98%</div>
              <div className="text-xs text-[#8A90A8] mt-2 flex items-center gap-1">
          ...
          <div className="grid grid-cols-3 gap-6">
            ...
            <div className="col-span-2 ai-card flex flex-col min-h-[400px]">
              ...
              <div className="flex-1">
                <div className="grid grid-cols-[2fr_1.5fr_1fr_1fr] px-5 py-3 text-xs font-medium text-[#8A90A8] border-b border-[#DCE2EF] uppercase tracking-wider">
                  <div>Athlete</div>
                  <div>Status / PB</div>
                  <div>Last Update</div>
                  ...
                  {athletes.map((athlete, i) => (
                    <div key={i} className="grid grid-cols-[2fr_1.5fr_1fr_1fr] items-center px-5 py-4 ai-table-row cursor-pointer group">
                      <div className="flex items-center gap-3">
                        <div className="relative">
                          <div className={`w-8 h-8 rounded bg-gradient-to-br flex items-center justify-center text-[10px] font-bold text-white shadow-inner
                            ${athlete.status === 'paused' ? 'from-zinc-700 to-zinc-800 text-[#6B7080]' : 'from-[#E75D50] to-[#B53C30]'}
                          `}>
                            {athlete.avatar}
                          </div>
                          {athlete.alert && (
                            <div className="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full bg-[#E75D50] border-2 border-[#FFFFFF]"></div>
                          )}
                        </div>
                        <div>
                          <div className={`text-sm font-medium flex items-center gap-2 ${athlete.status === 'paused' ? 'text-[#8A90A8]' : 'text-[#293055]'}`}>
                            {athlete.name}
                            {athlete.unread > 0 && (
                              <span className="bg-[#E75D50]/10 text-[#E75D50] text-[10px] px-1.5 py-0.5 rounded-sm font-semibold border border-[#E75D50]/20">
                                {athlete.unread} new
                              </span>
                            )}
                          </div>
                      ...
                      <div className="flex justify-end">
                        <button className="text-xs text-[#6B7080] hover:text-white px-3 py-1.5 rounded-md hover:bg-[rgba(41,48,85,0.06)] transition-colors border border-transparent hover:border-[#DCE2EF] opacity-0 group-hover:opacity-100">
                          Profile
                        </button>
                      </div>
                    </div>
            ...
            <div className="ai-card flex flex-col">
              ...
              <div className="p-5 flex-1 overflow-y-auto ai-scrollbar">
                  ...
                  {feedItems.map((item) => (
                    <div key={item.id} className={`ai-feed-item ${item.active ? 'active' : ''}`}>
                      ...
                      <div className="flex items-center gap-2 mb-1.5">
                        <span className="text-xs font-semibold text-[#293055]">{item.athlete}</span>
                        <span className="text-[10px] text-[#8A90A8] px-1.5 py-0.5 rounded bg-[rgba(41,48,85,0.06)] border border-[#DCE2EF]">
                          {item.category}
                        </span>
                        <span className="text-[10px] text-[#8A90A8] ml-auto">{item.time}</span>
                      </div>
                ...
                <button className="w-full mt-2 py-2 text-xs font-medium text-[#6B7080] hover:text-white transition-colors border border-dashed border-[#DCE2EF] rounded-md hover:bg-[rgba(41,48,85,0.06)] hover:border-white/20 flex items-center justify-center gap-1">
